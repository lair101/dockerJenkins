 ...
 *       <LI><TT>cmp.exerciser.IntegrationAPIExerciser.CommandInformation</TT>
 *     </UL>
 *   </TD>
 * </TR>
 * </TABLE>
...
public class GUIEventListener implements TreeSelectionListener, ActionListener, WindowListener, MouseListener {
     ...
     * @param exerciser
     */
    public GUIEventListener(IntegrationAPIExerciser exerciser) {
    ...
    private void maybeShowPopup(MouseEvent e) {
        if (e.isPopupTrigger()) {
            if (e.getSource() == exerciser.consoleLogger.getJTextComponent()) {
                ...
                try {
                    exerciser.addMenuItem(consoleMenu, ResourcesHandler.getNLSResource(ResourcesHandler.SAVE_CONSOLE),
                            new CommandInformation(ResourcesHandler.SAVE_CONSOLE, JFileChooser.SAVE_DIALOG, exerciser.consoleLogger.getSaveFileExtension(), false, exerciser, exerciser.getClass().getMethod("saveConsole", new Class[] { String.class } ), true));
                    exerciser.addMenuItem(consoleMenu, ResourcesHandler.getNLSResource(ResourcesHandler.CLEAR_CONSOLE),
                            new CommandInformation(null, null, null, null, false, exerciser.classTesterMisc,exerciser.classTesterMisc.getClass().getMethod("clearConsole", (Class[])null), true));
                } catch (NoSuchMethodException e1) {
                    e1.printStackTrace();
                }
                consoleMenu.show(e.getComponent(),
                        e.getX(), e.getY());
            } else if (e.getSource() == exerciser.table) {
                exerciser.tableMenu.show(e.getComponent(), e.getX(), e.getY());
            } else {
                if (exerciser.getConnectedBrokerProxyInstance() != null)    {
        ...
        if (o instanceof JTree) {
            ...
            if (t != null) {
                if (t.getLastPathComponent() instanceof DefaultMutableTreeNode) {
                    ...
                    // Look up the CMP object for this object
                    Object props = exerciser.nodesToCMPObjects.get(selectedObject);
                    exerciser.selectCMPObject(props);
                }
            }
        }
    }
        ...
        if (source == exerciser.cancelButton) {
            ...
            for (int i=0; i<activeCommandInformation.parametersRequired; i++) {
                ...
                if (i<exerciser.tf.length) {
                    ...
                    if (exerciser.tf[i] instanceof JTextField) {
                        parameters[i] = (String)((JTextField)(exerciser.tf[i])).getText();
                    } else if (exerciser.tf[i] instanceof JComboBox) {
                        parameters[i] = (String)((JComboBox)(exerciser.tf[i])).getSelectedItem();
                    } else if (exerciser.tf[i] instanceof JCheckBox) {
                        boolean isSelected = ((JCheckBox)(exerciser.tf[i])).isSelected();
                        parameters[i] = "" + isSelected;
                    } else {
                        parameters[i] = exerciser.tf[i].toString();
                    }
        ...
        if (activeCommandInformation != null) {
            ...
            if ((parameters == null) && (activeCommandInformation.parametersRequired > 0)) {
                ...
                exerciser.getParameters(
	                    ...
	                    defaultParameters,
                        activeCommandInformation.methodToGenerateInputBoxes,
