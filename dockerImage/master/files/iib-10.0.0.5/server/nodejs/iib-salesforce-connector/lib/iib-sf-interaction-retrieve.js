/**
 *
 * NAME: iib-sf-interaction-retrieve.js
 *
 * DECSRIPTION: Interaction-retrieve class for the IIB Salesforce Connector
 *
 *
...
//////////////////////////////////////////////////////////////////////////
// NAME: iibSFInteractionRetrieve
function iibSFInteractionRetrieve( dataModel, interaction, requestProperties, data, callback ) {
    ...
    if (typeof requestProperties.filterString !== 'undefined') {
        ...
        // A string containing te complete files has been supplied, so we
        // must convert it into a JSON object. 
        // Note: If the user has suppiled a "where" clause it is their
        //       responsibility to include the "__SOQL" wrapper  if they
        //       are specifying a SOQL format where clause.
        try {
        ...
        if (typeof requestProperties.filter.where !== 'undefined') {
            ...
            // The where clause is supplied as a SOQL string so we cannot pass
            // it directly as part of the where filter (loopback will reject
            // it as it is a string not an obect), so instead we place it in the
            // whereSOQL property and the 'Loopback Operation Hook' we installed
            // will rename the whereSOQL property to where.
            tofind.whereSOQL = requestProperties.filter.where; 
        }
    ...
    if (typeof requestProperties.id !== 'undefined') {
        ...
        if (typeof requestProperties.externalId === 'undefined') {
            ...
            // matching using  it. Note this will override any other 'whereSOQL'
            // where filter already specified.
            // Note: this mechanism does not validate the types specified, Salesforce
            // will do that.
            delete tofind.whereSOQL;
            delete tofind.where;
    ...
    // Activity Log
    iib.ActivityLog.writeTraceEvent('iib-sf-interaction', 'retrieve', 'BIP13052', 'Retrieve', requestProperties.object, requestProperties.url).resourceManager(interaction.connector.shortName).end();
    ...
    if (typeof findId !== 'undefined') {
        ...
        dataModel.findById( findId, tofind, function (err, rest) {
            ...
            if (err) {
                logger.debug("Failed to find record " + findId + ". Error name: " + err.name + " - " + err.message);
                 ...
                 // Activity Log
                 iib.ActivityLog.writeTraceEvent('iib-sf-interaction', 'retrieve', 'BIP13054', 'Retrieve', findId, requestProperties.object, err.name, err.message).resourceManager(interaction.connector.shortName).end();

                // Pass the error back to our caller
		ErrorObject = sfutils.createSFError( err, 'iib-sf-interaction', 'retrieve', 'Salesforce Retrieve using Id failed', err.name, 'Retrieve', err.message );
            } else if (rest === null) {
                logger.debug("iibSFInteractionRetrieveById: No record found for id: " + findId);

                // Activity Log
                iib.ActivityLog.writeTraceEvent('iib-sf-interaction', 'retrieve', 'BIP13053', 'Retrieve', requestProperties.object, findId).resourceManager(interaction.connector.shortName).end();
            } else {
                logger.debug("iibSFInteractionRetrieveById: Got result for id: " + findId);
        ...
        dataModel.find( tofind, function (err, rest) {
            ...
            if (err) {
                ...
                // Activity Log
                iib.ActivityLog.writeTraceEvent('iib-sf-interaction', 'retrieve', 'BIP13054', 'Retrieve', '', requestProperties.object, err.name, err.message).resourceManager(interaction.connector.shortName).end();

                // Pass the error back to our caller
                ErrorObject = sfutils.createSFError( err, 'iib-sf-interaction', 'retrieve', 'Salesforce Retrieve failed', err.name, 'Retrieve', err.message);
            }
            else {
                logger.debug("iibSFInteractionRetrieve: Got " + rest.length + " record(s) returned");
                // logger.debug(JSON.stringify(rest));

                // Activity Log
                iib.ActivityLog.writeTraceEvent('iib-sf-interaction', 'retrieve', 'BIP13053', 'Retrieve', requestProperties.object, '').resourceManager(interaction.connector.shortName).end();
...
module.exports = iibSFInteractionRetrieve;
